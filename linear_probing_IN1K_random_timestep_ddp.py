...
class Latent(torch.nn.Module):
    ...
    def forward_return_n_last_blocks(self, x, t=None, representation_layers=[], return_patch_avgpool=False):
        ...
        if not (isinstance(t, torch.Tensor) and t.ndim == 1 and t.shape[0] == x.shape[0] and t.device == x.device):
            t = torch.tensor([self.t]*x.shape[0]).to(x.device)      # if invalid t, use constant timestep
...
class DiffRep(torch.nn.Module):
    ...
    def forward(self, x, t=None):
        x = self.latent.forward_return_n_last_blocks(x, t, self.representation_layers, self.return_patch_avgpool)
        x = self.head(x)
        return x
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in loader_train:
            x = x.to(device)
            y = y.to(device)
            t = torch.randint(0, 1000, size=[x.shape[0]], dtype=int).to(device)
            y_pred = model(x, t)
